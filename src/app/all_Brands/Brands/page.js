...
export default function BrandsPage() {
  ...
  const token = localStorage.getItem("adminToken");
    ...
    try {
      const res = await fetch("http://localhost:4000/api/v1/createBrand", {
  ...
  const handleUpdateUnit = async (id) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:4000/api/v1/brand/${editId}`,
        {
          method: "PUT",
  ...
  useEffect(() => {
    async function GetAllvariation() {
      const res = await fetch(
        "http://localhost:4000/api/v1/totalBrands",
        {
          method: "GET",
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/api/v1/brand/${id}`, {
        method: 'DELETE',
        headers: {
          "Authorization": `Bearer ${token}`,
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await fetch(`http://localhost:4000/api/v1/brand/toggle/${id}`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`,
        },
