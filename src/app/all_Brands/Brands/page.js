...
export default function BrandsPage() {
  ...
  useEffect(() => {
    async function GetAllvariation() {
      const res = await fetch(
        "https://backend-customizer-1.onrender.com/api/v1/totalBrands",
        {
          method: "GET",
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`https://backend-customizer-1.onrender.com/api/v1/brand/${id}`, {
        method: 'DELETE',
        headers: {
          "Authorization": `Bearer ${token}`,
  ...
  const toggleStatus = async (id) => {
    try {
      const res = await fetch(`https://backend-customizer-1.onrender.com/api/v1/brand/toggle/${id}`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`,
        },
