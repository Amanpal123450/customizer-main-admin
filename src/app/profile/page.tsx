...
export default function Page() {
  ...
  const token = localStorage.getItem("adminToken");
      ...
      try {
        const res = await fetch(
          `https://backend-customizer-1.onrender.com/api/v1/getAdminById/${user}`,
          {
            headers: {
  ...
  const token = localStorage.getItem("adminToken");
    ...
    try {
      const res = await fetch(`https://backend-customizer-1.onrender.com/api/v1/updateprofile/${userId}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        },
