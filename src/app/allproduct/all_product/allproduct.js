...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        setLoading(true);
        const res = await fetch("https://backend-customizer-1.onrender.com/api/v1/totalProduct");
        if (!res.ok) {
  ...
  useEffect(() => {
    async function fetchDiscounts() {
      ...
      try {
        const res = await fetch("https://backend-customizer-1.onrender.com/api/v1/discounts", {
          headers: {
  ...
  const confirmed = await confirmDialog("Are you sure you want to delete this product?");
      ...
      const response = await fetch(
        `https://backend-customizer-1.onrender.com/api/v1/deleteProduct/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
