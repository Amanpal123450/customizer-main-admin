...
export default function UnitsPage() {
  const [Unitss, setBrands] = useState([]);
  ...
  useEffect(() => {
    const filteredData = Unitss.filter(
      (Units) =>
        Units.name.toLowerCase().includes(search.toLowerCase()) &&
        (status === "All" ||
          (status === "Active" && Units.active) ||
          (status === "Inactive" && !Units.active)),
    );
    setFiltered(filteredData);
    setCurrentPage(1);
  }, [search, status, Unitss]);
  ...
  const handleSave = () => {
    ...
    if (editing) {
      setBrands(
        Unitss.map((b) => (b.id === editing.id ? { ...b, ...form } : b)),
      );
    } else {
      const newBrand = {
        id: Date.now(),
      ...
      <div className="mx-auto max-w-7xl">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <nav className="mt-2 flex items-center space-x-2 text-sm text-gray-500">
                ...
                <span>Products</span>
                <span>/</span>
                <span className="font-medium text-indigo-600">Units</span>
              </nav>
            </div>
            <button
        ...
        <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {Unitss.filter((b) => b.active).length}
                </p>
              </div>
          ...
          <div className="rounded-lg bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">Inactive Units</p>
        ...
        <div className="mb-6 rounded-lg bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Search Unitss by name..."
                value={search}
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((Units, index) => (
                    ...
                    >
                      ...
                      {/* <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="w-12 h-12 bg-gray-100 rounded-lg overflow-hidden">
                            <img 
                              src={Units.logo || `https://ui-avatars.com/api/?name=${Units.name}&background=6366f1&color=fff`}
                              alt={Units.name}
                              className="w-full h-full object-cover"
                              onError={(e) => {
                                e.target.src = `https://ui-avatars.com/api/?name=${Units.name}&background=6366f1&color=fff`;
                              }}
                            />
                          </div>
                        </div>
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <button
                          onClick={() => toggleStatus(Units._id)}
                          className="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                          style={{
                            backgroundColor: Units.active
                              ? "#10b981"
                              : "#d1d5db",
                          }}
                        >
                          <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                              Units.active ? "translate-x-6" : "translate-x-1"
                            }`}
                        ...
                        <span
                          className={`ml-3 text-sm font-medium ${Units.active ? "text-green-600" : "text-red-600"}`}
                        >
                          {Units.active ? "Active" : "Inactive"}
                        </span>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            onClick={() => handleDelete(Units._id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="z-10 w-full max-w-md transform rounded-xl bg-white shadow-2xl transition-all">
            ...
            <div className="space-y-6 p-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <label className="text-sm font-medium text-gray-700">
                  Units is {form.active ? "active" : "inactive"}
                </label>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 border-t border-gray-200 p-6">
              ...
              >
                {editing ? "Update Units" : "Create Unit"}
              </button>
            </div>
          </div>
        </div>
