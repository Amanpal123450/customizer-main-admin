...
export default function AdminPage() {
  ...
  const token = localStorage.getItem("adminToken");
        ...
        const res = await fetch("https://backend-customizer.onrender.com/api/v1/getAllUsers", {
          headers: {
  ...
  const token = localStorage.getItem("adminToken");
      ...
      const res = await fetch(` https://backend-customizer.onrender.com/api/v1/deleteUser/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
