...
export default function CategoryDetailPage() {
  ...
  async function handleDelete(id) {
    const token = localStorage.getItem('adminToken');
    const res = await fetch(
      `https://backend-customizer-1.onrender.com/api/v1/deleteSubCategory/${id}`,
      {
        method: "DELETE",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("https://backend-customizer-1.onrender.com/api/v1/createSubCategory", {
        method: "POST",
        headers: {
