...
const OrderManagementPage = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch(
        " https://backend-customizer-1.onrender.com/api/v1/orders/all-orders",
      );
  ...
  const token = localStorage.getItem("adminToken");
      ...
      const res = await fetch(`https://backend-customizer-1.onrender.com/api/v1/ordersStatus/${orderId}`, {
        method: "PUT",
        headers: {
 ...
 console.log(selectedOrder)
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        const res = await fetch(
          `https://backend-customizer-1.onrender.com/api/v1/orders/${selectedOrder}`
        );
        if (!res.ok) {
